    ...
    <div>
        ...
        <div className="py-[50px] flex flex-col items-center px-[10px] md:px-[50px] lg:px-[100px] ">
            <div className="grid grid-cols-1 lg:grid-cols-2  gap-6 ">
             {
             displayedDonations.map((appliedDonation) => (<DonationCard key={appliedDonation.id}appliedDonation={appliedDonation} />
             ))
             }
            </div>
            <button className="btn bg-green-600 text-[18px] font-semibold text-white mt-[40px] py-3 w-[150px] hover:text-green-600 hover:font-bold hover:bg-white hover:border-2 hover:border-green-600" onClick={toggleShowAll}>{showAll ? "Show Less" : "See All"}</button>
            </div>     
        )
        }
        {
            ...
            <div className="flex flex-col items-center justify-center">
              ...
              <h1 className="text-4xl font-bold text-[#FF444A]">{`You haven't made any donations yet`}</h1>
            </div>
            )
        }
    </div>
